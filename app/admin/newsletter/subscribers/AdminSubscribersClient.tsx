...
export default function AdminSubscribersClient({
  ...
  initialData: { items: Sub[]; total: number } | null;
}) {
  const toast = useToast();
  const [items, setItems] = useState<Sub[]>(initialData?.items ?? []);
  ...
  useState(() => {
    if (initialData) {
      setCached(`subscribers?page=1&pageSize=${PAGE_SIZE}`, {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/newsletter/subscribers?${qs}`, { cache: "no-store" });
  ...
  const confirmDelete = async () => {
    ...
    setWorking(true);
    try {
      const res = await fetch(`/api/admin/newsletter/subscribers?id=${deleteTarget.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  const exportCsv = () => {
    ...
    if (query) params.set("q", query);
    window.open(`/api/admin/newsletter/subscribers?${params.toString()}`, "_blank");
  };
