...
export default async function ContentCalendarPage() {
  ...
  try {
    ...
    const ev: CalEvent[] = [];
    for (const r of rels) {
      if (r.releaseDate) ev.push({ id: `release-${r.id}`, type: "release", title: r.name, dateKey: dayKeyUtc(r.releaseDate), href: `/admin/catalog/releases/${r.id}/edit` });
    }
    for (const t of tasks) {
      if (t.dueAt && t.status !== "done") ev.push({ id: `task-${t.id}`, type: "task", title: t.title, dateKey: dayKeyUtc(t.dueAt), href: "/admin/tasks" });
    }
    for (const p of press) {
      if (p.publishedAt) ev.push({ id: `press-${p.id}`, type: "press", title: p.title, dateKey: dayKeyUtc(p.publishedAt), href: `/admin/catalog/press/${p.id}/edit` });
    }
    for (const c of campaigns) {
      const d = c.sentAt ?? c.scheduledFor;
      if (d) ev.push({ id: `campaign-${c.id}`, type: "newsletter", title: c.subject, dateKey: dayKeyUtc(d), href: "/admin/outreach/newsletter" });
    }
